...
# %%
df = pd.read_csv('C:/Projects/MasterSc/ML/Practice/Employee/Employee.csv')
data = df.copy()
...
# %%
mod_rnd_frs = (RandomForestClassifier(random_state=37).fit(X_res, y_res))
prd_rnd_frs = predict_with_measure(mod_rnd_frs, X_test, y_test)
               
# %%
mod_bag_knn = (BaggingClassifier(KNeighborsClassifier(),
                                 max_samples=0.75,
                                 max_features=0.75,
                                 random_state=37).fit(X_res, y_res))
prd_bag_knn = predict_with_measure(mod_bag_knn, X_test, y_test)

# %%
mod_ada_bst = (AdaBoostClassifier(algorithm='SAMME',random_state=37).fit(X_res, y_res))
prd_ada_bst = predict_with_measure(mod_ada_bst, X_test, y_test)

# %%
mod_grd_bst = (GradientBoostingClassifier(learning_rate=0.3,subsample=0.75,
                                          max_features='sqrt',
                                          random_state=37).fit(X_res, y_res))
...
estimators = [('lnr', clf1),('knn', clf2),('gnb', clf3)]

mod_vot_clf = VotingClassifier(estimators=estimators,voting='soft').fit(X_res, y_res)
...
mod_stk_clf = StackingClassifier(estimators=estimators,final_estimator=final_estimator).fit(X_res, y_res)

prd_stk_clf = predict_with_measure(mod_stk_clf, X_test, y_test)

# %%
scores = pd.DataFrame.from_dict(f1_scores,orient='index',columns=['f1', 'time'])
