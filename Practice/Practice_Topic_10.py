...
# %%
with open ('C:\Projects\MasterSc\ML\Practice\mod_05_topic_10_various_data.pkl', 'rb') as fl:
...
autos[['stroke', 'bore', 'stroke_ratio']].head(10)
...
# %%
customer = (customer.assign(StateFreq=lambda x: 
                            x.groupby('State')['State']
                            .transform('count') /x['State'].count()))
...
# %%
mi_scores = mutual_info_regression(X, y,discrete_features=X.columns.isin(
    cat_features.to_list() +['num_of_doors','num_of_cylinders']),random_state=37)
mi_scores = (pd.Series(
     mi_scores,
     name='MI Scores',
     index=X.columns)
